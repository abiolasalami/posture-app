...
try:
    ...
    def main():
        ...
        if 'ALERT_DELAY' not in st.session_state:
            st.session_state.ALERT_DELAY = 5
        ...
        # Bouton pour activer/désactiver l'analyse
        ...
        # Zone principale pour la vidéo et le traitement
        video_placeholder = st.empty()
        
        # Utiliser st.camera_input() au lieu de cv2.VideoCapture
        if st.session_state.running:
            # Utiliser camera_input pour obtenir l'image de la webcam
            camera_image = st.camera_input("Webcam", key="posture_webcam")
            
            if camera_image is not None:
                # Convertir l'image en format OpenCV
                image = np.array(Image.open(camera_image))
                
                # Traiter l'image avec MediaPipe (qui attend RGB)
                ...
                video_placeholder.image(image_with_results, channels="RGB", use_container_width=True)
        else:
        ...
        3. Une alerte sonore retentit après quelques secondes de mauvaise posture
        4. Vous pouvez ajuster le délai d'alerte et activer/désactiver le son
        """)
        
        # Notes au bas de page
        st.sidebar.markdown("---")
        st.sidebar.info("⚠️ Pour que l'application fonctionne correctement, assurez-vous d'autoriser l'accès à la webcam quand le navigateur le demande.")
